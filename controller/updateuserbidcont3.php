...
include_once 'handler/aesconfig.php'; 
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $encryptionKey = "heitech-village-2"; // Replace with your key
    $encryptedBidID = $_POST['BidID'];
    $decryptedBidID = decrypt($encryptedBidID, $encryptionKey);

    if ($decryptedBidID === false) {
        http_response_code(400);
        echo "<script>alert('Invalid BidID provided.');</script>";
        exit();
    }

    // Retrieve and sanitize POST data
    $bidID = intval($decryptedBidID); // Use the decrypted BidID
    ...
    $debugMessages[] = "Decoded BidID: $bidID";
